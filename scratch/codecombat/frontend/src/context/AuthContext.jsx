...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(localStorage.getItem('codecombat_token'));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const initAuth = async () => {
      const storedToken = localStorage.getItem('codecombat_token');
      if (storedToken) {
        try {
  ...
  const login = (authData) => {
    localStorage.setItem('codecombat_token', authData.token);
