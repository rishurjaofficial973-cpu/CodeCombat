...
export const MonacoCodeEditor = ({
  ...
  const [selectedLanguage, setSelectedLanguage] = useState(() => {
    return localStorage.getItem('codecombat_preferred_language') || 'JAVA';
  });
  ...
  useEffect(() => {
    ...
    const draftKey = `codecombat_draft_${problemId}_${selectedLanguage}`;
  ...
  const handleLanguageChange = (newLang) => {
    ...
    localStorage.setItem('codecombat_preferred_language', newLang);

    const draftKey = `codecombat_draft_${problemId}_${newLang}`;
    const savedDraft = localStorage.getItem(draftKey);
    if (savedDraft) {
      setCode(savedDraft);
